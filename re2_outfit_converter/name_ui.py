    ...
    *,
    ...
    for outfit in checked:
        ...
        if (choice or "").startswith("Delete"):
            continue
        target = resolve_target(choice)
        if target is not None and target.msg_stem:
            return (
...
def collect_display_names_by_target(
    ...
    """Map target outfit key → display name for convert ops with msg_stem."""
    out: dict[str, str] = {}
    for op in ops:
        target = getattr(op, "target", None)
        source = getattr(op, "source", None)
        if target is None or source is None or not target.msg_stem:
            continue
