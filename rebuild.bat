...
if not exist "Build\RE2 Outfit Converter" mkdir "Build\RE2 Outfit Converter" >nul 2>&1
rem Preserve local prefs / output created by running the Build copy.
robocopy "dist\RE2 Outfit Converter" "Build\RE2 Outfit Converter" /MIR /XF settings.json /XD Output /NFL /NDL /NJH /NJS /nc /ns /np >nul
set "RC=%ERRORLEVEL%"
if %RC% GEQ 8 (
  echo ERROR: Failed to sync dist to Build ^(robocopy %RC%^).
  exit /b 1
)
