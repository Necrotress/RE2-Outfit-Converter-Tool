...
if [[ ! -f "$ROOT/convert.sh" ]]; then
  echo "ERROR: convert.sh not found. Run this from the extracted package folder."
  exit 1
fi
...
do_convert() {
  ...
  echo
  echo "Done. Check: $out"
  echo "Drop the new .zip into Fluffy's RE2R Mods folder (do not extract)."
  pause
}
